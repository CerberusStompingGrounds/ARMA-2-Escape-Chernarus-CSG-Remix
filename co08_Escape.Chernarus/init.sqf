...
diag_log ("call compile preprocessFileLineNumbers ""Scripts\Escape\Functions.sqf"";");

[_isJipPlayer] call compile preprocessFileLineNumbers "Briefing.sqf";
diag_log ("[" + str _isJipPlayer + "] call compile preprocessFileLineNumbers ""Briefing.sqf"";");
call compile preprocessFileLineNumbers "scripts\Init_UPSMON.sqf";
diag_log ("call compile preprocessFileLineNumbers ""scripts\Init_UPSMON.sqf"";");

_dynamicWeather = (paramsArray select 3);
diag_log ("_dynamicWeather = " + str (paramsArray select 3) + ";");
...
if (isServer) then {
    ...
    diag_log ("execVM ""InitServer.sqf"";");
    if (isDedicated) exitWith {};
};

waitUntil {drn_var_Escape_playerEnteredWorld};
diag_log ("waitUntil {drn_var_Escape_playerEnteredWorld};");
...
diag_log ("drn_var_Escape_syncronizationDone = true;");
publicVariable "drn_var_Escape_syncronizationDone";
player sideChat "All clients synchronized!";

waitUntil {!isNil "drn_var_Escape_FunctionsInitializedOnServer"};
diag_log ("waitUntil {!isNil ""drn_var_Escape_FunctionsInitializedOnServer""};");
[] call drn_fnc_Escape_AskForTimeSynchronization;
diag_log ("[] call drn_fnc_Escape_AskForTimeSynchronization;");
...
diag_log ("Removed all weapons for player.");
